...
class TerrierIndexBuilder:
    ...
    def build(
        ...
        """Return (index, indexing_time_seconds)."""
        key = cfg.key()
        if key in self._cache and not force:
            return self._cache[key]

        path = os.path.join(self.index_root, f"idx_{key}")
        if os.path.exists(path):
            shutil.rmtree(path)
        os.makedirs(path, exist_ok=True)

        indexer = self._make_indexer(path, cfg)
...
class RocchioConfig:
    ...
    def label(self) -> str:
        return (f"Rocchio(a={self.alpha},b={self.beta},g={self.gamma},"
                f"k={self.fb_docs},m={self.fb_terms})")


class RocchioExpander:
    """Second-pass retrieval with a centroid-shifted query vector."""
    ...
    def search(self, tokens: Sequence[str], k: int = 1000) -> List[Tuple[str, float]]:
        ...
        if not qvec:
            return []
        first_pass = self.vsm.score_vector(qvec, max(k, cfg.fb_docs + cfg.neg_docs))
        if len(first_pass) < 2:
            return first_pass[:k]

        expanded: Dict[str, float] = {t: cfg.alpha * w for t, w in qvec.items()}

        positives = first_pass[: cfg.fb_docs]
        ...
        negatives = first_pass[-cfg.neg_docs:] if cfg.neg_docs else []
        negatives = [d for d in negatives if d not in positives]
        ...
        # Truncate to the strongest terms; original query terms are protected.
        expanded = {t: w for t, w in expanded.items() if w > 0.0}
        if cfg.fb_terms and len(expanded) > cfg.fb_terms:
            keep = dict(
                sorted(expanded.items(), key=lambda kv: -kv[1])[: cfg.fb_terms]
            )
            for term in qvec:
                if term in expanded:
                    keep.setdefault(term, expanded[term])
            expanded = keep

        norm = math.sqrt(sum(w * w for w in expanded.values())) or 1.0
        expanded = {t: w / norm for t, w in expanded.items()}
        return self.vsm.score_vector(expanded, k)
...
class ExperimentPaths:
    ...
    def indexes(self) -> str:
        return os.path.join(self.workdir, "indexes")

    @property
    def runs(self) -> str:
        return os.path.join(self.workdir, "runs")
...
def run_experiments(args: argparse.Namespace) -> None:
    ...
    if args.skip_tuning:
        ...
        best_rocchio = RocchioConfig(1.0, 0.75, 0.15, 10, 20, 10)
